...
- Routes:
  ...
  - `POST /api/contactos` — formulario de tasación (web) o ChatBot; acepta los `name` del form React (`metros`, `acepta`, `tipo` como string) y los mapea a `metros_cuadrados`, `acepta_novedades`, `tipo_id`. Requiere `nombre` y **email o teléfono** (el ChatBot puede enviar solo teléfono). Acepta `origen` (`'ChatBot' | 'Web'`, default `'Web'`).
  - `GET /api/contactos` — bandeja de consultas del panel admin (auth requerida); en Mongo `.sort({ fecha: -1 }).populate('tipo_id', 'nombre_tipo')`, en memoria revierte el array y defaultea `origen: 'Web'` a los docs viejos del seed.
  - `DELETE /api/contactos/:id` — elimina una consulta (auth requerida); Mongo `findByIdAndDelete` (404 si id inválido/inexistente), memoria splice por `_id`.
- **Sesiones**: tokens aleatorios (`crypto.randomBytes(24)`) en un `Map` en memoria, **persistidos en `data/sesiones.json`** (gitignored) al emitirlos y recargados al arrancar → los tokens del panel siguen válidos tras reiniciar el servidor. El frontend los guarda en `localStorage['nt_sesion']`; ante un 401 (token viejo/inválido) cierra sesión y vuelve al login.
...
- `Contacto`: `nombre`, `email` (**opcional** — el ChatBot puede enviar solo `telefono`), `telefono`, `zona`, `metros_cuadrados`, `mensaje`, `acepta_novedades` (Boolean), `fecha` (default `Date.now`), `tipo_id` → ref `TipoPropiedad`, `origen` (String, default `'Web'`, valores `'Web'` | `'ChatBot'`).
...
- **`/admin`** (`AdminPanel.jsx`, ruta protegida): topbar con "Ver sitio público" / "Cerrar sesión" y formulario completo de alta (`direccion`, `zona`, `ambientes`, `metros_cuadrados`, `precio`, `operacion`, `tipo_id` y `agente_id` desde APIs, `caracteristicas` coma-separadas → array). **Imágenes con drag & drop**: zona `admin-drop` (click para elegir o arrastrar archivos png/jpg/gif/webp) → cada archivo se sube vía `POST /api/imagenes` (FileReader → data-URL, con `Authorization: Bearer` del usuario en sesión) y aparece como miniatura con input de **etiqueta** (datalist con sugerencias: Frente, Patio, Balcón, Cocina, Ambiente Principal) + botón Quitar; al publicar se envían `imagenes` (urls `/uploads/...`) y `imagenes_etiquetas` en paralelo. Si la subida responde 401 (sesión realmente vencida) cierra sesión y redirige a `/login` en vez de dejar el error en la vista. Envía `POST /api/properties` con `Authorization: Bearer <token>`; ante 401 cierra sesión y vuelve al login.
- **Gestión de Propiedades** (debajo del form, `AdminPanel.jsx`): listado de `GET /api/properties` **sin límite alguno** (trae y renderiza TODAS las propiedades; el "5" que se ve es simplemente la cantidad del seed) con miniatura, dirección, zona, precio USD y botón **Eliminar** (rojo). Justo arriba tiene un **input de búsqueda en tiempo real** (`placeholder: "Buscar propiedad a eliminar por dirección, barrio o tipo..."`) que filtra por dirección, zona/barrio o tipo. Al clickear Eliminar pide confirmación con `window.confirm("¿Seguro que querés eliminar esta propiedad de la base de datos?")` y, si se acepta, hace `DELETE /api/properties/:id` con el token y refresca la lista; el listado también se refresca tras cada alta.
- **Bandeja de Consultas y Cotizaciones** (debajo de Gestión, `AdminPanel.jsx`): carga `GET /api/contactos` con `Authorization: Bearer` al montar (401 → cierra sesión y va a `/login`). Grid de tarjetas (`.admin-consultas`) con nombre, **badge de Origen** (`admin-origen--chatbot` azul / `admin-origen--web` gris; los docs viejos sin `origen` se muestran como "Web"), email · teléfono, mensaje, pie con tipo (resuelto desde `tipo_id` poblado **o** lookup en `tipos` en modo memoria), zona y fecha (`toLocaleString('es-AR')`), y botón **Eliminar** → `window.confirm` → `DELETE /api/contactos/:id` → refresca. Se refresca también al montar.
- **`ContactForm.jsx`** (sección `#tasacion`): ahora es dinámico — carga `GET /api/tipos-propiedad` para resolver `tipo_id` del select (`Casa/Departamento/Oficina` → id; `Otro` → manda `tipo` string) y al enviar hace `POST /api/contactos` con FormData (`nombre, email, telefono, tipo, zona, metros, mensaje, acepta` → `{..._cuadrados, acepta_novedades}`). Muestra feedback `.contacto-mensaje--ok/--error` y resetea el form al éxito.
- **`ChatBot.jsx` + `ChatBot.css`**: asistente flotante (esquina inferior derecha, `position: fixed`, z-index 1200) renderizado solo en Home (`App.jsx`, después de `<Footer/>`). Al abrir muestra diálogo con bienvenida + botones rápidos **"Tasación / Cotización"** y **"Consulta de propiedad"**; flujo asistido por pasos (`inicio → nombre → contacto → mensaje`): input con placeholder según paso, detecta si el contacto es email (`@`) o teléfono y manda `POST /api/contactos` con `{ nombre, email|telefono, mensaje: "<tipoConsulta> — <mensaje>", origen: 'ChatBot' }`. Botón ↺ reinicia la conversación; si el POST falla permite reintentar. En el Home solo se ve el FAB 💬 (click → diálogo).
